...
int main() {
    ...
    double two_opt_savings = 0;
    double three_opt_savings = 0;
    ...
    for (int iter = 0; iter < 20; iter++) {
        ...
        std::cout << "Three Opt changes: " << three_opt_nn(coords, neighbors, route, checklist, iter == 0, CHECKLIST_SIZE) << std::endl;
        auto end = std::chrono::steady_clock::now();
        elapsed_time += end - start;

        double three_opt_route_len = 0;
        for (int i = 1; i < NODES; i++) {
            three_opt_route_len += get_distance(coordinates[route[i]], coordinates[route[i - 1]]);
        }
        std::cout << "Post Three Opt route distance: " << three_opt_route_len << std::endl;

        double two_opt_route_len = 0;
        for (int i = 1; i < NODES; i++) {
            two_opt_route_len += get_distance(coordinates[ls_info_list[iter].corrected_route[i]], coordinates[ls_info_list[iter].corrected_route[i - 1]]);
        }
        std::cout << "Correct Two Opt route distance: " << two_opt_route_len << std::endl;
        two_opt_savings += route_len - two_opt_route_len;
        three_opt_savings += route_len - three_opt_route_len;
    }

    std::cout << "Elapsed time: " << elapsed_time.count() << std::endl;
    std::cout << "Improvement over 2 opt: " << (three_opt_savings/two_opt_savings) << std::endl;
}